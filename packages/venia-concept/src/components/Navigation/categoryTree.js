...
class Tree extends Component {
        ...
        return rootNodeId ? (
            <Query query={navigationMenu} variables={{ id: rootNodeId }}>
                {({ loading, error, data }) => {
                    ...
                    // let temp = children[children.length - 1];
                    // temp.id = temp.id + 1;
                    // temp.name = 'Custom 1';
                    // children.push(temp);
                    ...
                    let extraMenu = this.getCustomMenu();

                    return (
                        <Fragment>
                            <div className={branchClass}>{leaves} {extraMenu}</div>
                            {branches}
                        </Fragment>
                    );
                }}
    ...
    getCustomMenu() {
        ...
        return (
            <li>
                <a className="categoryLeaf-root-2AM" href="/data.html">
                    <span className="categoryLeaf-text-36T">BLOG</span>
                </a>
            </li>
        )
    }
